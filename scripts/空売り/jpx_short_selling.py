...
def parse_date_like(v: Any) -> dt.date | None:
    ...
    if isinstance(v, (int, float)):
        n = int(v)
        if 20000 < n < 80000:  # Excel シリアル値の想定レンジ
...
def fetch_file_index() -> dict[dt.date, str]:
    ...
    pattern = re.compile(r"(\d{8})_Short_Positions\.xlsx?$", re.IGNORECASE)
...
def load_dataframe(data: bytes) -> pd.DataFrame:
    ...
    head = data[:8]
    if head[:4] == b"\xd0\xcf\x11\xe0":          # OLE2 = 旧 .xls
        engine = "xlrd"
    elif head[:2] == b"PK":                       # ZIP = .xlsx
        engine = "openpyxl"
    else:                                          # HTML テーブルの場合
...
def detect_header(df: pd.DataFrame) -> tuple[int, dict[str, int]]:
    ...
    required = {"コード", "空売り残高割合", "空売り残高数量"}
    if not required.issubset(best_map):
        raise RuntimeError(
...
def build_records(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rec = rec[rec["code"].str.fullmatch(r"[0-9A-Z]{4,5}")].copy()
    if rec.empty:
